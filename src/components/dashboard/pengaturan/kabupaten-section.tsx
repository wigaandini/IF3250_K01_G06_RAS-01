...
export default function WilayahKabupatenSection() {
  const [items, setItems] = useState<Kabupaten[]>([]);
  const [provinsis, setProvinsis] = useState<Provinsi[]>([]);
  const [loading, setLoading] = useState(true);
  const [showForm, setShowForm] = useState(false);
  const [form, setForm] = useState<Partial<Kabupaten>>({});
  ...
  const fetchKabupaten = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/wilayah/kabupaten?page=${currentPage}&limit=${itemsPerPage}&sortBy=${sortBy}&search=${encodeURIComponent(
          search
        )}`
      );
  ...
  }, [currentPage, itemsPerPage, sortBy, search, showToast]);

  const fetchProvinsi = async () => {
    try {
      const res = await fetch("/api/wilayah/provinsi?page=1&limit=1000");
      const result = await res.json();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.status === 409) {
        showToast("Kode kabupaten sudah ada.", "error");
        return;
      }
      ...
      console.error(err);
      showToast("Gagal menyimpan data kabupaten", "error");
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/wilayah/kabupaten/${toDeleteId}`, {
        method: "DELETE",
      });
      ...
      showToast("Gagal menghapus kabupaten", "error");
      console.error(err);
    } finally {
      setToDeleteId(null);
    }
  ...
  const handleEdit = (kab: Kabupaten) => {
    setForm({
      id: kab.id,
      kode: kab.kode,
      nama: kab.nama,
    ...
    <>
        ...
        <CardHeader className="pb-2">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
            <div className="flex items-center gap-4">
              <CardTitle className="text-lg w-100">Data Kabupaten</CardTitle>
              <Input
                  ...
                  {items.length === 0 ? (
                    ...
                    items.map((k) => (
                      <TableRow key={k.id}>
                        ...
                        <TableCell>{k.provinsi?.nama || "-"}</TableCell>
                        <TableCell>
      ...
      <Dialog open={showForm} onOpenChange={setShowForm}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>
              {form.id ? "Edit Kabupaten" : "Tambah Kabupaten"}
            </DialogTitle>
          </DialogHeader>
          <div className="grid gap-3 pt-2">
            ...
            <Select
              ...
              onValueChange={(v) =>
                setForm({ ...form, provinsi_id: parseInt(v) })
              }
            >
          ...
          <div className="flex justify-end gap-2 pt-4">
            ...
            <Button
              ...
              disabled={!form.nama || !form.provinsi_id}
      ...
      >
        <DialogContent className="sm:max-w-md">
          ...
          <p>Yakin ingin menghapus kabupaten ini?</p>
          <div className="flex justify-end gap-2 pt-4">
